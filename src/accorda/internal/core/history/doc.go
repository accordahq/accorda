...
// `accorda history`.
//
// A Receipt is the immutable record of a successful deployment
// (docs/ACCORDA.md §7): the deployment identifier, repository, environment,
// commit, start/completion timestamps, and per-service image reference and
// resolved manifest digest. The digest is the point of the receipt — Git may
// declare a mutable tag, but Accorda records the immutable digest so it can
// answer "exactly which commit and image digest was running at time Y?".
//
...
//
// See docs/ACCORDA.md §5.2 (Deployed State), §7 (Deployment Receipts), §20
// (Rollback), and the `accorda history` CLI surface in §45 for the
