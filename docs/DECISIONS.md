...
| `github.com/docker/docker` | v28.5.2+incompatible | `internal/targets/compose` | Docker engine API client used by `Target.Current` to list and inspect the project's containers and map them to `state.RuntimeService` (container state + health). Reached through a local `dockerClient` seam so the SDK does not leak into core. |
...
label into a `state.RuntimeState`. The project name is derived from the
